...
EOF"
    ...
    # Compile on Romeo
    log_info "Compiling on Romeo..."
    remote_exec "cd ${REMOTE_BASE_DIR} && source setup_env.sh && make clean && make sequential && make parallel"
    ...
    echo "  3. Submit jobs: bash scripts/romeo/run_benchmarks.sh"
    echo ""
    log_info "Or run from here:"
    echo "  ./scripts/romeo/run_benchmarks.sh --submit"
}
