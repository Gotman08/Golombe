...
Le présent travail vise à concevoir et implémenter un solveur de règles de Golomb hautement optimisé et parallélisé, capable de s'exécuter efficacement sur le cluster HPC Romeo de l'Université de Reims. Les objectifs spécifiques sont :
...
Ce rapport suit la progression naturelle de l'optimisation, des fondements théoriques jusqu'aux architectures distribuées les plus avancées :

\begin{itemize}[leftmargin=*]
    \item \textbf{Chapitre 2} : Fondements algorithmiques --- formalisation du problème et présentation de l'algorithme Branch-and-Bound avec ses mécanismes d'élagage.

    \item \textbf{Chapitre 3} : Implémentation séquentielle (V1) --- optimisation mono-cœur avec instructions AVX2 et structure de données BitSet256, établissant la \textit{baseline} de performance.

    \item \textbf{Chapitre 4} : Parallélisation OpenMP (V2) --- exploitation du parallélisme de tâches et résolution des problèmes de synchronisation (race condition, false sharing, contention atomique).

    \item \textbf{Chapitre 5} : Distribution MPI Master/Worker (V3) --- passage à l'échelle multi-nœuds et identification des limitations de l'architecture centralisée.

    \item \textbf{Chapitre 6} : Architecture Hypercube décentralisée (V4/V5) --- élimination du goulot d'étranglement par distribution statique et propagation de bornes en $O(\log P)$.

    \item \textbf{Chapitre 7} : Résultats expérimentaux --- analyse du scaling fort et faible sur le cluster Romeo, comparaison des versions.

    \item \textbf{Chapitre 8} : Discussion --- analyse critique des performances, limitations identifiées et perspectives d'amélioration.

    \item \textbf{Chapitre 9} : Conclusion --- synthèse des apprentissages et recommandations pour les praticiens du HPC.
\end{itemize}
...
\subsection{Métriques de Performance Parallèle}

Avant d'aborder les versions parallélisées, il convient de définir formellement les métriques utilisées pour évaluer leur efficacité.

\begin{definition}{Speedup et Efficacité Parallèle}{speedup-def}
Soit $T_1$ le temps d'exécution sur un processeur et $T_P$ le temps sur $P$ processeurs.

Le \textbf{speedup} (accélération) est défini par :
\[
S(P) = \frac{T_1}{T_P}
\]

L'\textbf{efficacité parallèle} mesure l'utilisation des ressources :
\[
E(P) = \frac{S(P)}{P} = \frac{T_1}{P \cdot T_P}
\]

Une efficacité de 100\% signifie un speedup linéaire parfait ($S(P) = P$), rarement atteignable en pratique.
\end{definition}

\begin{theoreme}{Loi d'Amdahl}{amdahl}
Soit $s$ la fraction \textit{intrinsèquement séquentielle} du programme (non parallélisable). Le speedup maximal atteignable avec $P$ processeurs est borné par :
\[
S(P) \leq \frac{1}{s + \frac{1-s}{P}} \xrightarrow[P \to \infty]{} \frac{1}{s}
\]

Cette loi établit une limite théorique au speedup : même avec un nombre infini de processeurs, l'accélération ne peut dépasser $1/s$.
\end{theoreme}

\textbf{Implication pratique} : Pour un algorithme Branch-and-Bound, la fraction séquentielle inclut la génération initiale des sous-arbres et la synchronisation finale. Si $s = 5\%$, le speedup maximal théorique est $1/0.05 = 20\times$, indépendamment du nombre de processeurs utilisés.
...
\textbf{Analyse} : Chaque nœud de l'arbre de recherche effectue une lecture atomique de \texttt{globalBestLength} pour vérifier si l'élagage est possible. Avec des millions de nœuds par seconde et 32 threads, cela génère une contention massive sur le bus mémoire.

Le mécanisme sous-jacent est le \textbf{cache line bouncing} : la variable atomique réside sur une ligne de cache unique. Chaque lecture avec \texttt{memory\_order\_acquire} force une synchronisation avec la dernière écriture, provoquant des invalidations en cascade via le protocole de cohérence de cache. Sur une architecture NUMA à deux sockets, ce trafic traverse le lien inter-socket (AMD Infinity Fabric), ajoutant une latence de 100+ nanosecondes par accès.
...
\textbf{Analyse} : Le \textbf{false sharing} constitue l'un des pièges les plus insidieux de la programmation parallèle sur architectures à mémoire partagée. Ce phénomène se produit lorsque des threads sur différents cœurs accèdent à des données distinctes mais situées sur la même \textit{ligne de cache} (64 octets sur les processeurs AMD Zen 3).

Pour maintenir la \textbf{cohérence de cache}, les processeurs modernes implémentent le protocole \textbf{MESI} (Modified, Exclusive, Shared, Invalid). Lorsqu'un cœur modifie une donnée, il doit d'abord \textit{invalider} les copies de cette ligne de cache présentes dans les caches des autres cœurs. Ce mécanisme, appelé \textit{cache line bouncing}, génère un trafic inter-cœurs massif même si les threads modifient des variables différentes --- d'où le terme ``faux'' partage.
...
\begin{tikzpicture}[scale=0.9]
    % Lignes de vie
    \draw[thick] (0, 0) -- (0, -5) node[below] {Rang 0};
    \draw[thick] (6, 0) -- (6, -5) node[below] {Rang 1};

    % Labels en haut
    \node[above] at (0, 0) {\textbf{Rang 0}};
    \node[above] at (6, 0) {\textbf{Rang 1}};

    % Envoi synchrone Rang 0 -> Rang 1
    \draw[->, thick, red] (0, -1) -- (5.8, -1.5) node[midway, above, sloped, font=\small] {MPI\_Ssend};
    \node[left, font=\tiny] at (0, -1) {t=1};

    % Envoi synchrone Rang 1 -> Rang 0
    \draw[->, thick, red] (6, -1.3) -- (0.2, -1.8) node[midway, below, sloped, font=\small] {MPI\_Ssend};
    \node[right, font=\tiny] at (6, -1.3) {t=1};

    % Zone de blocage
    \fill[red!20, opacity=0.5] (-0.3, -2) rectangle (0.3, -4.5);
    \fill[red!20, opacity=0.5] (5.7, -2) rectangle (6.3, -4.5);

    % Labels de blocage
    \node[font=\tiny, rotate=90] at (-0.5, -3.25) {BLOQUÉ};
    \node[font=\tiny, rotate=90] at (6.5, -3.25) {BLOQUÉ};

    % Annotations
    \draw[decorate, decoration={brace, amplitude=5pt, mirror}] (-0.8, -2) -- (-0.8, -4.5);
    \node[left, font=\tiny, text width=2cm, align=right] at (-1, -3.25) {Attend que\\Rang 1 reçoive};

    \draw[decorate, decoration={brace, amplitude=5pt}] (6.8, -2) -- (6.8, -4.5);
    \node[right, font=\tiny, text width=2cm] at (7, -3.25) {Attend que\\Rang 0 reçoive};

    % Deadlock symbol
    \node[font=\Large\bfseries, red] at (3, -3.5) {DEADLOCK};
\end{tikzpicture}
\caption{Diagramme de séquence illustrant le deadlock avec MPI\_Ssend}
\label{fig:mpi-deadlock}
\end{figure}

\textbf{Explication technique} : MPI distingue deux modes d'envoi selon la taille du message :

\begin{itemize}
    \item \textbf{Mode eager} (messages courts, typiquement $<$ 4--64 KB selon l'implémentation) : le message est copié dans un buffer interne et l'envoi retourne immédiatement.

    \item \textbf{Mode rendezvous} (messages longs ou envoi synchrone \texttt{MPI\_Ssend}) : l'émetteur attend une confirmation du récepteur avant de transmettre, garantissant que le récepteur est prêt.
\end{itemize}

Avec \texttt{MPI\_Ssend}, les deux processus attendent simultanément une confirmation qui ne viendra jamais, créant une \textbf{dépendance circulaire}.

\begin{solution}{Utilisation de MPI\_Send standard}{mpi-send}
La solution consiste à utiliser \texttt{MPI\_Send} en mode standard, qui utilise automatiquement le mode \textbf{eager} pour les petits messages :
...
void broadcastBound(int bound) {
    for (int neighbor : neighbors) {
        MPI_Send(&bound, 1, MPI_INT, neighbor,
                 TAG_BOUND, MPI_COMM_WORLD);
        // Mode eager : buffer interne, retour immediat
        // Pas de synchronisation bloquante
    }
}
\end{lstlisting}

Pour un entier (4 octets), MPI utilise systématiquement le mode eager : le message est copié dans un buffer interne et l'appel retourne immédiatement, éliminant le risque de deadlock.

\textbf{Note} : Une alternative serait d'utiliser \texttt{MPI\_Isend} (envoi non-bloquant), mais cela nécessiterait une gestion explicite des requêtes avec \texttt{MPI\_Wait}, ajoutant de la complexité sans bénéfice pour des messages de 4 octets.
\end{solution}
...
\section{Limitations et Analyse Critique}

\subsection{Analyse de l'Efficacité Décroissante}

Les résultats de scaling faible (\cref{tab:weak-scaling}) révèlent une diminution de l'efficacité de 100\% à 61.8\% lorsque le nombre de workers passe de 8 à 128. Cette dégradation s'explique par plusieurs facteurs :

\begin{enumerate}
    \item \textbf{Déséquilibre de charge inhérent} : Les sous-arbres générés à la profondeur \texttt{prefixDepth} ont des tailles très variables. Certains sont élagués rapidement (borne atteinte), tandis que d'autres nécessitent l'exploration de millions de nœuds. La distribution statique ne peut pas compenser cette hétérogénéité.

    \item \textbf{Coût de synchronisation} : L'opération \texttt{MPI\_Allreduce} finale pour déterminer la meilleure solution globale a une complexité $O(\log P)$ en latence, mais le temps absolu augmente avec $P$. Pour 128 processus, cette synchronisation représente environ 3.5\% du temps total.

    \item \textbf{Effet de la propagation des bornes} : Une borne trouvée par un processus met $O(\log_2 P)$ étapes pour atteindre tous les autres. Pendant ce délai, certains processus explorent des branches déjà obsolètes. Ce phénomène s'amplifie avec $P$.

    \item \textbf{Overhead de génération redondante} : Chaque rang génère l'intégralité des sous-arbres avant de sélectionner sa portion, induisant un travail redondant proportionnel au nombre de sous-arbres.
\end{enumerate}

\subsection{Comparaison avec les Limites Théoriques}

En appliquant la loi d'Amdahl avec une fraction séquentielle estimée à $s \approx 2\%$ (génération des sous-arbres + synchronisation finale), le speedup maximal théorique serait $1/0.02 = 50\times$. Le speedup observé de 91x pour V4 \textbf{dépasse} cette limite théorique, ce qui s'explique par un phénomène de \textbf{speedup superlinéaire}.

\begin{tcolorbox}[colback=yellow!5, colframe=yellow!50!black, title=\textbf{Speedup Superlinéaire}]
Le speedup superlinéaire ($S(P) > P$) peut survenir lorsque :
\begin{itemize}
    \item La propagation rapide des bornes permet d'élaguer des branches qui auraient été explorées en séquentiel
    \item Les données tiennent mieux dans les caches L2/L3 distribués
    \item La recherche parallèle trouve plus rapidement une bonne borne initiale
\end{itemize}
Dans notre cas, c'est principalement le premier facteur qui explique ce phénomène.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{Équilibrage de charge statique} : V4/V5 distribuent les sous-arbres uniformément, mais certains sous-arbres peuvent être 100x plus coûteux que d'autres, créant des \textit{stragglers} (processus retardataires).

    \item \textbf{Mémoire} : Chaque rang génère tous les sous-arbres avant de sélectionner sa portion, ce qui peut consommer plusieurs gigaoctets pour les ordres élevés.

    \item \textbf{Sensibilité au paramètre \texttt{prefixDepth}} : Un choix sous-optimal de la profondeur de génération des sous-arbres dégrade significativement les performances. Ce paramètre doit être ajusté empiriquement selon l'ordre.
\end{itemize}
...
Le présent travail a permis de concevoir, implémenter et évaluer un solveur de règles de Golomb hautement performant, évoluant d'une version séquentielle optimisée vers une architecture distribuée scalable. Les contributions principales sont :

\begin{itemize}
    \item \textbf{Optimisation vectorielle} : L'exploitation des instructions AVX2 pour la détection de collisions a permis d'obtenir une accélération de 1.6x à 2.5x par rapport à une implémentation scalaire, validant l'intérêt des instructions SIMD pour les opérations sur bitsets.

    \item \textbf{Parallélisation mémoire partagée} : La version OpenMP atteint un speedup de 53x sur 64 threads avec une efficacité parallèle de 83\%, démontrant la viabilité du parallélisme de tâches pour les algorithmes Branch-and-Bound.

    \item \textbf{Architecture décentralisée} : L'architecture hypercube (V4) atteint un speedup de 91x sur 128 workers, surpassant significativement l'architecture master/worker centralisée et confirmant l'hypothèse selon laquelle la décentralisation élimine les goulots d'étranglement de communication.

    \item \textbf{Validation expérimentale} : Les solutions optimales ont été vérifiées jusqu'à l'ordre 13 (longueur 106), en accord avec les résultats de référence de la littérature \cite{oeis}.
\end{itemize}

\section{Enseignements et Principes Généraux}

Au-delà des résultats numériques, ce travail illustre plusieurs principes fondamentaux du calcul haute performance :

\begin{enumerate}
    \item \textbf{Primauté du profilage} : L'identification des goulots d'étranglement par mesure objective (profilage avec \texttt{perf}) révèle souvent des causes inattendues. La contention atomique, représentant 45\% du temps CPU, n'aurait pas été détectée par simple inspection du code.

    \item \textbf{Supériorité des architectures décentralisées} : Pour les problèmes à granularité irrégulière, les topologies sans point central de contention (hypercube) surpassent les architectures centralisées dès que le nombre de processus dépasse une dizaine.

    \item \textbf{Nature itérative de l'optimisation} : Chaque niveau d'optimisation résout les limitations du précédent tout en révélant de nouveaux défis. Cette progression \textit{séquentiel → multi-thread → distribué centralisé → distribué décentralisé} constitue un schéma récurrent dans le développement d'applications HPC.

    \item \textbf{Impact des détails d'implémentation} : Les choix apparemment mineurs --- alignement mémoire sur 64 octets, intervalle de rafraîchissement de la borne, mode d'envoi MPI --- peuvent avoir un impact de plusieurs ordres de grandeur sur les performances.
\end{enumerate}

\section{Recommandations pour les Praticiens}

Sur la base de l'expérience acquise, les recommandations suivantes peuvent être formulées pour l'implémentation d'algorithmes Branch-and-Bound parallèles sur architectures HPC :

\begin{enumerate}
    \item \textbf{Établir une baseline fiable} : Avant toute parallélisation, implémenter et optimiser une version séquentielle rigoureusement testée. Cette version sert de référence pour le calcul des speedups et permet d'isoler les problèmes algorithmiques des problèmes de parallélisation.

    \item \textbf{Réduire la contention sur les variables partagées} : Le caching des bornes globales avec rafraîchissement périodique offre un excellent compromis entre fraîcheur de l'information et débit. L'intervalle optimal dépend de la granularité des nœuds et doit être déterminé empiriquement.

    \item \textbf{Privilégier la décentralisation à grande échelle} : Au-delà de 16-32 processus, les architectures sans point central de contention (hypercube, tore) surpassent systématiquement les modèles master/worker.

    \item \textbf{Mesurer avant d'optimiser} : Le profilage systématique avec des outils appropriés (\texttt{perf}, \texttt{VTune}, tracing MPI) doit précéder toute optimisation. Les intuitions sur les goulots d'étranglement sont souvent erronées.
\end{enumerate}
...
\bibitem{gendron1994}
B.~Gendron and T.~G.~Crainic, ``Parallel Branch-and-Bound Algorithms: Survey and Synthesis,'' \emph{Operations Research}, vol.~42, no.~6, pp.~1042--1066, 1994.

\bibitem{kumar1983}
V.~Kumar and L.~N.~Kanal, ``A General Branch and Bound Formulation for Understanding and Synthesizing And/Or Tree Search Procedures,'' \emph{Artificial Intelligence}, vol.~21, no.~1-2, pp.~179--198, 1983.

\bibitem{seitz1985}
C.~L.~Seitz, ``The Cosmic Cube,'' \emph{Communications of the ACM}, vol.~28, no.~1, pp.~22--33, 1985.

\bibitem{fox1988}
G.~C.~Fox, M.~A.~Johnson, G.~A.~Lyzenga, S.~W.~Otto, J.~K.~Salmon, and D.~W.~Walker, \emph{Solving Problems on Concurrent Processors}, vol.~1, Prentice-Hall, 1988.

\bibitem{eggers1989}
S.~J.~Eggers and R.~H.~Katz, ``The Effect of Sharing on the Cache and Bus Performance of Parallel Programs,'' in \emph{Proc. 3rd International Conference on Architectural Support for Programming Languages and Operating Systems (ASPLOS)}, pp.~257--270, 1989.

\bibitem{torrellas1990}
J.~Torrellas, M.~S.~Lam, and J.~L.~Hennessy, ``Shared Data Placement Optimizations to Reduce Multiprocessor Cache Miss Rates,'' in \emph{Proc. International Conference on Parallel Processing}, pp.~266--270, 1990.
